...
function renderGroupPicker() {
    ...
    wrap.innerHTML = `
        ...
        <div class="grp-cards" id="gGroupCards">
            ${groups.map(g => `
                <button class="grp-card" data-id="${g.id}" onclick="selectGroup('${g.id}')">
                    <div class="grp-card-icon">${groupEmoji(g.type)}</div>
                    <div class="grp-card-name">${g.name}</div>
                    <div class="grp-card-sub">${g.count} ${g.type}${g.herd ? ' · ' + g.herd : ''}</div>
                </button>
            `).join('')}
            <button class="grp-card grp-card-manual" onclick="selectManual()">
...
function groupSavedFromGrazing(newGroupId) {
    window._addingGroupFromGrazing = false;
    renderGroupPicker();
    if (newGroupId) selectGroup(newGroupId);
}
...
function saveGroup() {
    ...
    if (!type) { alert('Select an animal type.'); return; }
